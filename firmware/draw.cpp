...
volatile bool aquisition, hold, rangeV = true, rangeI = true;
...
void drawVoltScreen(void)
{
	while (mode == MODE_VOLTAGE)
	{
		...
		lcd.setFont(&Lato_Medium_9);
		lcd.printxy(aquisition ? (char*)"V DC" : (char*)"V RMS", 50, 31);
		lcd.printxy(rangeV ? (char*)"60 V" : (char*)" 6 V", 60, 40);
		lcd.setCursor(0, 48);
...
void drawCurrScreen(void)
{
	while (mode == MODE_CURRENT)
	{
		...
		lcd.printxy(rangeI ? (char*)"500 mA" : (char*)"60 mA", 46, 40);
		lcd.setCursor(0, 48);
		lcd.printf((char*)"%4.2f V", adc1_result_vsup);
		lcd.setCursor(51, 48);
		lcd.printf((char*)"%4.2f V", adc2_getReading(VBAT) * 2 * (adc1_result_vsup / 4096.0));
