...
void SDADC_init(void)
{
	...
	SDADC1->CR2 |= (6 << 16);						//select channel 8 (voltage) for SDADC1
	SDADC2->CR2 |= (8 << 16);						//select channel 6 (current) for SDADC2
	
	SDADC1->CR2 |= SDADC_CR2_FAST;			//FAST mode
	SDADC2->CR2 |= SDADC_CR2_FAST;
	
	/*SDADC1->CONF0R |= SDADC_CONF0R_COMMON0_0;		//set common mode for calibration
	SDADC2->CONF0R |= SDADC_CONF0R_COMMON0_0;*/
	
	//CONF0 - no changes needed
